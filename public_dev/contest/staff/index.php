...
foreach ($dashboard as $t) {
    ...
    foreach ($t["contests"] as $c) {
        $html .= "<li><div class=\"flex\" style=\"justify-content:space-between;\"><p>{$c["status"]}</p><p>{$c["name"]}</p><p>{$c["start"]}~{$c["end"]}</p><p>discussion:{$c["discussion"]}</p></div><ul>";
        foreach ($c["problems"] as $p) {
            $html .= "<li><div class=\"flex\" style=\"justify-content:space-between;\"><p>{$p["title"]}(id: {$p["name"]})</p><p>target:{$p["file_name"]}</p></div><ul>";
            foreach ($p["users"] as $u) {
                ...
                foreach ($u["submits"] as $s) {
                    $html .= "<li><a href=\"scoring?submit_id={$s["id"]}\"><div class=\"flex\" style=\"justify-content:space-between;\"><p>{$s["status"]}</p><p>{$s["user_name"]}</p><p>{$s["file_name"]}</p><p>{$s["submitted_at"]}</p></div></a></li>";
                }
                $html .= "</ul></li>";
            }
            $html .= "</ul></li>";
...
<html lang="ja">
  ...
  <body>
	...
	<a href="/TSC/contest/staff/"><h1>ようこそ、STAFF様</h1></a>
	<div>
	<a href="/TSC/contest/staff/score"><p>採点する</p></a>
        <div>
            <?=$html?>
        </div>
	</div>
