...
if($level < TSC::LEVEL_STAFF && $status == TSC::STATUS_UPCOMING){
...
while($problem = $stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    if(!$isguest && $status == TSC::STATUS_ONGOING)$contents_html .=
    "                    <div style=\"width: 100%;text-align: center;\">
                            <form action=\"problems.php\" method=\"post\" enctype=\"multipart/form-data\" >
                                表示名:<input type=\"text\" name=\"user_name\" value=\"{$user["name"]}\" required>
                                <input type=\"hidden\" name=\"submit_name\" value=\"{$problem["name"]}\" style=\"display:none;\" required>
                                <input type=\"file\" name=\"upload_{$problem["name"]}\" onchange=\"previewFile(this, 'preview_{$problem["name"]}');\" required>
                                <input type=\"submit\" value=\"送信する\">
                            </form>
                            <p>プレビュー</p>
                            <img id=\"preview_{$problem["name"]}\" style=\"width:60%;\">
                        </div>";
    $contents_html .= "</div>";
}

if(!$isguest && $status == TSC::STATUS_ONGOING){
